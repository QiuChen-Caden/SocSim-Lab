...
start "SocSim Backend (OneClick)" cmd /k "cd /d \"%BACKEND_DIR%\" && if not exist venv python -m venv venv && call venv\\Scripts\\activate.bat && if not exist venv\\Lib\\site-packages\\fastapi pip install -r requirements.txt && set OASIS_DB_PATH=%DATA_DIR%\\oasis_frontend.db && set PYTHONIOENCODING=utf-8 && python main.py"
